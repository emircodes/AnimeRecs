...
animes_df = pd.read_csv('AnimeRecs/anime.csv', sep='\t')
...
''' Splitting the genres because of the "|" separator '''
#this makes the values inside the column become a list
animes_df['genres'] = animes_df['genres'].str.split('|')

#unique genre for all rows
#creates a set for all the genres in the genres column
unique_genres = set(genre for genres in animes_df['genres'] for genre in genres)

# one-hot encoding process
for genre in unique_genres:
    animes_df[genre] = animes_df['genres'].apply(lambda genres: 1 if genre in genres else 0)

# Drop the original 'genre' column
animes_df = animes_df.drop(columns=['genres'])

# combining the features
combined_features = np.hstack([tfidf_matrix.toarray(), animes_df[list(unique_genres)].values])
...
similarity_distance = 1 - cosine_similarity(combined_features)
